...
async function startNgrok() {
    ...
    // Check for static domain in env or args
    const staticDomain = process.env.NGROK_DOMAIN;
    const args = ['http', FRONTEND_PORT.toString()];

    if (staticDomain) {
        // Sanitize domain: remove protocol and trailing slashes
        const sanitizedDomain = staticDomain.replace(/^https?:\/\//, '').replace(/\/$/, '');
        console.log(`Using static domain: ${sanitizedDomain}`);
        args.push(`--domain=${sanitizedDomain}`);
    }
