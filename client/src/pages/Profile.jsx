...
function Profile() {
    ...
    const [stats, setStats] = useState({ solvedCount: 0, points: 0, rank: '', username: '' });
    ...
    const fetchProfile = async () => {
        try {
            ...
            const u = res.data;
            setStats({ solvedCount: u.solvedCount, points: u.points, rank: u.rank, username: u.username });
            setFormData({
                bio: u.bio || '',
                photo: u.photo || '',
            ...
            <div className="card flex items-center gap-6 mb-8">
                <div
                    style={{
                        ...
                        backgroundImage: `url(${formData.photo})`, backgroundSize: 'cover', backgroundPosition: 'center',
                        display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden'
                    }}
                >
                    {!formData.photo && <span className="text-2xl font-bold">{stats.username?.[0]?.toUpperCase()}</span>}
                </div>
                <div className="flex-grow">
                    <h3 className="text-2xl font-bold">{stats.username}</h3>
                    <div className="flex gap-4 mt-2 text-sm">
                        <span className="badge badge-blue">{stats.rank}</span>
                        <span className="badge badge-green">{stats.points} PTS</span>
                        <span className="badge" style={{ borderColor: '#555' }}>{stats.solvedCount} SOLVES</span>
                    </div>
